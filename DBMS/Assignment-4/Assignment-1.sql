...
DELIMITER $$
CREATE FUNCTION get_month_with_maximum_orders(input_year INT)
RETURNS VARCHAR(10)
DETERMINISTIC 
BEGIN
    ...
    INTO result_month
    FROM(
        SELECT Month(date) AS month_of_year, COUNT(id) AS total_orders
        FROM orders
        WHERE YEAR(date) = input_year
        GROUP BY month_of_year
        ) AS inner_table
    WHERE total_orders = MAX(total_orders);
    RETURN result_month;
END $$

SELECT get_month_with_max_order(2018) AS "month with max orders in 2018";
